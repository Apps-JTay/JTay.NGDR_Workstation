...
namespace NGDR_Workstation
{
    public partial class DR_Workstation : Form
    {
        ...
        public DR_Workstation(string passedLoc, bool passesPro, bool passedEnc, int encnum, bool passedUnzip)
        {
            InitializeComponent();
            location = passedLoc;
        ...
        private static int ApptDays
        {
            get
            {
                string temp = GetPath("Appt");
                return StringUtility.IsNumeric(temp, NumberStyles.Number) ? Convert.ToInt32(temp) : 1;
            }
        }
        ...
        {
            string zipFile = "EMR_" + dateToUse.ToString("MM_dd_yyyy") + ".jtzip";
            ArrayList filesToZip = new ArrayList();

            foreach (string file in Directory.GetFiles(tempDir))
            {
                FileInfo info = new FileInfo(file);
                if (info.Extension != ".jtzip")
                {
                    filesToZip.Add(file);
                    Log log = new Log(ssDataLayer)
                    {
